...
* The account-confirmation landing page is deployed as a public Edge Function at
  `https://ionjpertlpylsoshjmjm.supabase.co/functions/v1/auth-confirmation`.
  Its URL is allowed in **Supabase Dashboard → Authentication → URL
  Configuration → Additional Redirect URLs**. It gives the athlete a clear
  success screen and an explicit button to resume in the Android app.
